...
Produces the review manifest consumed by the trusted follow-up comment job. The
diff is computed on translator-meaningful identity only -- (msgctxt, msgid,
plural) -- so that source-location churn and header metadata never show up as
noise.
...
class Entry(dict):
    """A single source-string entry; serializes to the manifest `entry` shape."""

    def __init__(self, msgid: str, msgctxt: Optional[str], plural: bool):
        super().__init__(msgid=msgid, msgctxt=msgctxt, plural=plural)
...
def _entry(record: dict) -> Entry:
    return Entry(record["msgid"], record["msgctxt"], record["plural"])
